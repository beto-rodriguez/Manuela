...
public partial class AppMenu : Border
{
    ...
    {
        // close all the collapsed menus
        foreach (var item in AppRoutes.MainMenu.Where(x => x.CollapsedMenu?.IsOpen ?? false))
            item.CollapsedMenu!.Close();
    ...
    private void CloseMoreMenu(StackLayout item)
    {
        //MoreOptionsMenu.Close();
    }

    private void OnItemTapped(StackLayout item)
    {
        ActivateItem(item);

        if (item.BindingContext is not IMenuItem menuItem) return;
        menuItem.CollapsedMenu?.Toggle();
    }
}
